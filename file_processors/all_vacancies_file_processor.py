...
class AllVacanciesFileProcessor(FileProcessorBaseClass):
    ...
    def save_vacancies_to_file(self, *args: str, **kwargs: str) -> None:
        ...
        :param args: имена файлов, из которых загружать данные и в который сохранять
        :type args: str
        :param kwargs: имена файлов, из которых загружать данные и в который сохранять
        :type kwargs: str
        """
        filename: str = kwargs["filename"]
        hh_filename: str = kwargs["hh_filename"]
        sj_filename: str = kwargs["sj_filename"]
        hh_data: list[dict] = HeadHunterFileProcessor().load_vacancies_from_file(filename=hh_filename)
        sj_data: list[dict] = SuperJobFileProcessor().load_vacancies_from_file(filename=sj_filename)
